...
class HelpCommand(commands.HelpCommand):
    ...
    async def send_command_help(self, command):
        embed = discord.Embed(title=f'Help: {command.qualified_name}', color=self.context.bot.config['colors']['default'])
    ...
    async def send_group_help(self, group):
        embed = discord.Embed(title=f'Help: {group.qualified_name}', color=self.context.bot.config['colors']['default'])
        if group.short_doc:
    ...
    async def send_cog_help(self, cog):
        embed = discord.Embed(title=f'Help: {cog.qualified_name}', color=self.context.bot.config['colors']['default'])
        if cog.description:
